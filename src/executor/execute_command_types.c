...
/*   Updated: 2025/09/08 10:24:33 by makhudon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int get_exit_code(t_execute_data *data, t_process_data *process_data)
{
    ...
    {
        ft_error_with_arg("exit", data->clean_args[1], "numeric argument required");
        return 2;
    }
    else if (data->clean_args[2] != NULL)
    {
 ...
 *
 * This is the main orchestrator function. It handles argument cleanup and redirection,
 * then dispatches the command to the appropriate handler (either the built-in
								...
								t_process_data *process_data)
{
	...
	if (ft_strncmp(data.clean_args[0], "exit", 5) == 0)
		...
		exit_status = execute_builtin_or_external(&data, process_data, env_list);
	process_data->last_exit_status = exit_status;
	free_execute_data(&data);
	return exit_status;
}
