...
/*   Updated: 2025/08/29 19:58:37 by makhudon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int export_variable(const char *arg, t_env_var *env_list)
{
    char        *key;
    char        *equal_sign;
    char        *clean;        /* final value to store, no re-expansion */
    t_env_var   *existing_var;
    int         append;
    ...
    if (equal_sign == NULL)
    {
        ...
        if (existing_var == NULL)
        {
            ...
            if (!key)
                return (1);
            /* store as *unset* (NULL), so `export` prints `declare -x NAME` */
            add_env_var(key, NULL, env_list);
            free(key);
        }
        return (0);
    ...
    /* -------- NO MORE EXPANSION HERE --------
       Parser/expander already handled quotes/expansion, so take RHS as-is. */
    clean = ft_strdup(equal_sign + 1);
    if (!clean) { free(key); return (1); }
    ...
    if (existing_var)
    {
        if (append)
        {
            ...
            {
                /* append to unset behaves like simple set */
                existing_var->value = clean;  /* take ownership */
            }
        }
        else
